...
public class HarLogger : IHarLogger
{
        ...
        string correlationId)
    {
        var entry = new Entry()
        {
            ...
            Request =
            {
                ...
                Url = new Uri($"{request.RequestUri!}-{correlationId}"),
    ...
    private async Task FlushToFile()
    {
        ...
        var har = HarEntrypoint.Deserialize(_harJson);
        ...
        _harJson = HarEntrypoint.Serialize(har);
    ...
    private void CreateNewLogFile()
    {
        try
        {
            ...
            _harJson = HarEntrypoint.Serialize(_harObj);
        }
        catch(Exception)
        {
        }
        ...
        HttpRequest request)
    {
        var requestMessage = new HttpRequestMessage()
        {
            ...
            Method = request.Method.IsEqual(nameof(HttpMethod.Get)) ?
                    HttpMethod.Get 
                    : HttpMethod.Post ,
        };
        ...
        HttpResponse response)
    {
        var responseMessage = new HttpResponseMessage(
            response.StatusCode.ToEnum<HttpStatusCode>());
        ...
        if(responseMessage.Headers.HasNo(h => h.Key.IsEqual("date")))
            responseMessage.Headers.Add(
                "Date" ,
                new string[] { clock.UtcNow.ToString("ddd, dd MMM yyyy HH:mm:ss zzzz") });
